...
import { updateName, updateAge, toggleCheckbox } from './store/actions';
...
const handlers = (dispatch) => ({
    onNameChanged: ({ target }) => dispatch(updateName(target.value)),
    onAgeChanged: ({ target }) => dispatch(updateAge(target.value)),
    onIsStudentChanged: () => dispatch(toggleCheckbox())
});

export default connect(stateToProps, handlers)(UserForm);